package it.gotoandplay.smartfoxserver.handlers
{
   ...
   public class SysHandler implements IMessageHandler
   {
      ...
      public function handleUserLeaveRoom(param1:Object) : void
      {
         ...
         var _loc4_:Room;
         if(!(_loc4_ = this.sfs.getRoom(_loc3_)) || !_loc4_.getUser(_loc2_))
         {
            return;
         }
      ...
      public function handleUserVarsUpdate(param1:Object) : void
      {
         ...
         if(param1.body.vars.toString().length > 0)
         {
            for each(_loc5_ in this.sfs.getAllRooms())
            {
               if((_loc4_ = _loc5_.getUser(_loc2_)) != null)
               {
                  _loc3_ = [];
                  this.populateVariables(_loc4_.getVariables(),param1.body,_loc3_);
               }
            }
            if(!this.sfs.getActiveRoom())
            {
               return;
            }
            (_loc6_ = {}).user = this.sfs.getActiveRoom().getUser(_loc2_) != null ? this.sfs.getActiveRoom().getUser(_loc2_) : _loc4_;
            _loc6_.changedVars = _loc3_;
